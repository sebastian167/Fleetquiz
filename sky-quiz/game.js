...
// ── Profanity Filter ───────────────────────────
// Comprehensive list for blocking inappropriate names (substring-matched)
const PROFANITY_LIST = [
  // Common English profanity
  "fuck", "shit", "bitch", "bastard", "dick", "cock",
  "pussy", "cunt", "whore", "slut", "fag", "nigger", "nigga", "retard",
  "piss", "tits", "boob", "penis", "vagina", "anus", "douche",
  // Common leetspeak / evasion variants
  "f u c k", "s h i t", "b i t c h", "d i c k",
  "fck", "fuk", "fuq", "phuck", "phuk", "sh1t", "sht", "b1tch",
  "btch", "d1ck", "dck", "azz", "cnt",
  // Racial slurs and hate speech
  "spic", "chink", "kike", "wetback", "gook", "wop", "beaner",
  "honky", "nazi", "aryan",
  // Sexually explicit
  "blowjob", "handjob", "dildo", "porn", "xxx", "anal", "cum",
  "jizz", "orgasm", "erect", "horny",
  // Other offensive
  "murder", "rape", "molest", "pedo", "terrorist",
  "suicide", "stfu", "gtfo",
];
...
function containsProfanity(text) {
  ...
  for (const word of PROFANITY_LIST) {
    ...
    // Skip words that become too short after stripping (avoids false positives)
...
function addLeaderboardEntry(entry) {
  ...
  saveLeaderboard(lb);
  return lb.findIndex(e => e === entry) + 1; // return rank (1-indexed, may return 0 if trimmed)
}
...
const DOM = {
  ...
  goLeaderboardMsg: $("go-leaderboard-msg"),
  goLbRankText:     $("go-lb-rank-text"),
  ...
  btnClearLeaderboard: $("btn-clear-leaderboard"),
};

// ── Game State ─────────────────────────────────
let state = {};
let currentPlayer = { name: "Pilot", station: "---" };
...
function gameOver() {
  ...
  setTimeout(() => {
    ...
    if (rank > 0 && rank <= LB_MAX) {
      DOM.goLeaderboardMsg.classList.remove("hidden");
      if (rank === 1) {
        DOM.goLbRankText.textContent = `🥇 NEW #1! You're the top pilot!`;
      } else if (rank <= 3) {
        const medals = ["", "🥇", "🥈", "🥉"];
        DOM.goLbRankText.textContent = `${medals[rank]} You placed #${rank} on the leaderboard!`;
      } else {
        DOM.goLbRankText.textContent = `📋 You placed #${rank} on the leaderboard`;
      }
    } else {
      DOM.goLeaderboardMsg.classList.add("hidden");
    }
...
function renderLeaderboard() {
  ...
  lb.forEach((entry, i) => {
    ...
    if (entry.name === currentPlayer.name && entry.station === currentPlayer.station) {
      tr.classList.add("lb-current-player");
    }
    ...
    tr.innerHTML = `
      ...
      <td><span class="lb-pilot-name">${escapeHTML(entry.name)}</span></td>
      <td style="text-align:center;"><span class="lb-station-tag">${escapeHTML(entry.station)}</span></td>
      <td class="lb-score-value">${entry.score.toLocaleString()}</td>
      <td class="lb-level-value">${entry.level}</td>
    `;
...
function validateAndStartGame() {
  const nameInput = DOM.playerName;
  const stationInput = DOM.playerStation;
  const name = nameInput.value.trim();
  const station = stationInput.value.trim().toUpperCase();

  // Reset error states
  nameInput.classList.remove("input-error");
  stationInput.classList.remove("input-error");
  ...
  // Validate station
  if (!station) {
    stationInput.classList.add("input-error");
    stationInput.focus();
    return;
  }
...
function init() {
  ...
  // Restore player info from last session
  ...
  if (savedStation) DOM.playerStation.value = savedStation;

  // ── Event Listeners ────────────────────────────
  DOM.btnStart.addEventListener("click", validateAndStartGame);
  ...
  // Leaderboard buttons
  DOM.btnLeaderboard.addEventListener("click", openLeaderboard);
  DOM.btnViewLeaderboard.addEventListener("click", openLeaderboard);
  ...
  DOM.playerStation.addEventListener("input", () => {
    DOM.playerStation.classList.remove("input-error");
  });

  // Allow Enter key to start game from inputs
  [DOM.playerName, DOM.playerStation].forEach(input => {
  ...
  // Keyboard support (1-4 for answers, Escape to close leaderboard)
